package com.yk.silence.locationpicker.ui;
...
import com.yk.silence.locationpicker.R;
import com.yk.silence.locationpicker.citywheel.CityConfig;
import com.yk.silence.locationpicker.citywheel.CityParseHelper;
import com.yk.silence.locationpicker.impl.CanShow;
import com.yk.silence.locationpicker.impl.OnCityItemClickListener;
import com.yk.silence.locationpicker.impl.OnWheelChangedListener;
...
import com.yk.silence.locationpicker.ui.wheelview.WheelView;
import com.yk.silence.locationpicker.ui.wheelview.adapter.ArrayWheelAdapter;
import com.yk.silence.locationpicker.util.ParseUtil;
...
public class CityPickerView implements CanShow, OnWheelChangedListener {
    ...
    private void initCityPickerPopwindow() {
        ...
        if (parseHelper.getProvinceBeanArrayList().isEmpty()) {
            Toast.makeText(context, "请在Activity中增加init操作",Toast.LENGTH_SHORT).show();
            return;
        }

        LayoutInflater layoutInflater = LayoutInflater.from(context);
        popview = layoutInflater.inflate(R.layout.item_picker_layout, null);
        ...
        popwindow.setOnDismissListener(new PopupWindow.OnDismissListener() {
            ...
            public void onDismiss() {
                if (config.isShowBackground()) {
                    ParseUtil.setBackgroundAlpha(context, 1.0f);
                }
            }
        });
        /**
        ...
        if (config != null && config.isShowBackground()) {
            ParseUtil.setBackgroundAlpha(context, 0.5f);
        }
     ...
     *
     */
    private void getProArrData(ProvinceBean[] array) {
        List<ProvinceBean> provinceBeanList = new ArrayList<>(Arrays.asList(array));
        //不需要港澳台数据
        if (!config.isShowGAT()) {
    ...
    private void setUpData() {
        ...
        if (!config.getCustomItemLayout().equals(CityConfig.NONE) && !config.getCustomItemTextViewId().equals(CityConfig.NONE)) {
            ...
            arrayWheelAdapter.setItemResource(R.layout.item_default_city);
    ...
    private void updateCities() {
        ...
        if (!config.getCustomItemLayout().equals(CityConfig.NONE) && !config.getCustomItemTextViewId().equals(CityConfig.NONE)) {
            ...
            cityWheel.setItemResource(R.layout.item_default_city);
    ...
    private void updateAreas() {
        ...
        if (config.getWheelType() == CityConfig.WheelType.PRO_CITY
            ...
            CityBean mCityBean = Objects.requireNonNull(parseHelper.getPro_CityMap().get(parseHelper.getProvinceBean().getName()))[pCurrent];
            ...
            if (config.getWheelType() == CityConfig.WheelType.PRO_CITY_DIS) {
                ...
                if (!config.getCustomItemLayout().equals(CityConfig.NONE)
                    ...
                    districtWheel.setItemResource(R.layout.item_default_city);
    ...
    public void onChanged(WheelView wheel, int oldValue, int newValue) {
        if (wheel == mViewProvince) {
            ...
            if (parseHelper != null && parseHelper.getCity_DisMap() != null) {
                ...
                DistrictBean mDistrictBean = Objects.requireNonNull(parseHelper.getCity_DisMap()
